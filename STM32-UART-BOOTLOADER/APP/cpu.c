...
void Wdt_Init(void)
{
    ...
    IWDG_WriteAccessCmd(IWDG_WriteAccess_Enable);  //IWDG->KR = 0x5555  允许看门狗寄存器写入功能
    
    // IWDG counter clock: 40KHz(LSI) / 256 = 156 Hz(6.4ms) 
    IWDG_SetPrescaler(IWDG_Prescaler_256);   //IWDG->PR = 0x06;  看门狗时钟分频
    
    // Set counter reload value to 1250   5s/6.4ms=781
    IWDG_SetReload(781);           //IWDG->RLR =0xFFF    喂狗时间
    
    // Red IWDG counter
    IWDG_ReloadCounter();   //IWDG->KR = 0xAAAA
    
    // Enable IWDG (the LSI oscillator will be enabled by hardware) 
    IWDG_Enable();   //IWDG->KR = 0xCCCC  启动看门狗
}
...
{
    //Reload IWDG counter  
    IWDG_ReloadCounter();   //IWDG->KR = 0xAAAA  使  IWDG->RLR中的值重新加载到计数器中，避免产生看门狗复位       
}
